...
class PaginatorTests(testcase.TestCase):
    ...
    def test_structure__little_pages(self):
        ...
        self.assertEqual(paginator.pages_numbers, list(range(4 * D)))

    def test_structure__left_hole(self):
        missed = 3
        paginator = self.create_paginator(4 * D + missed, 2 * D + missed)
        self.assertEqual(
            paginator.pages_numbers,
            list(range(D + 1)) + [None] + list(range(D + missed, 4 * D + missed)),
        )
    ...
    def test_structure__right_hole(self):
        ...
        self.assertEqual(
            ...
            list(range(3 * D + 1))
            + [None]
            + list(range(3 * D + missed - 1, 4 * D + missed)),
        )
    ...
    def test_structure__two_holes(self):
        ...
        self.assertEqual(
            ...
            list(range(D + 1))
            + [None]
            + list(range(D + missed, D + missed + 2 * D + 1))
            + [None]
            + list(range(3 * D + 2 * missed - 1, 4 * D + 2 * missed)),
        )
